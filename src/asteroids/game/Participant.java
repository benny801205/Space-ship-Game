...
import asteroids.participants.Score;
...
public abstract class Participant
{
    ...
    public void move ()
    {
        ...
        if(this instanceof Ship) {
            Shape original_flame = getflameShape();
            // Translate and rotate the original to reflect the accumulated motion
           
            flameshape = trans.createTransformedShape(original_flame);
    ...
    protected abstract Shape getflameShape();
    
    protected abstract Color getflameC();
    
    protected abstract void flamemove();
    ...
    public void draw (Graphics2D g)
    {
        ...
        if(this instanceof Ship) {
            g.draw(border);
            g.setPaint(getflameC());
            if(flameshape==null) {
                
                flameshape = getflameShape();
            }
            g.fill(flameshape);
            //g.draw(border);
        }
        
        else if(this instanceof Score|| this instanceof MAlienship) {
            g.fill(border);
        }
        else{g.draw(border);}
       // g.fill(shape);
